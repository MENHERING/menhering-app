...
// 효과음이 꺼져 있으면(useSoundStore) 재생만 건너뛰고 로드는 그대로 한다 —
// 로드까지 막으면 사용자가 켠 직후 첫 소리가 늦게 난다(아래 preload 주석 참고).
export function useSound(src: string, volume = 0.5) {
  ...
  return useCallback(() => {
    // 렌더가 아니라 재생 시점에 읽는다: 구독하면 토글 한 번에 효과음을 쓰는 화면이 전부 리렌더되고,
    // localStorage에서 복원한 값이 서버 마크업과 달라 하이드레이션 불일치가 날 수 있다.
    if (!useSoundStore.getState().isSfxEnabled) return;
