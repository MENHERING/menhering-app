...
서버 `avatar_status`는 감정을 **`mood_value` 정수 하나(0~100)**로만 준다. `Mood` 5단계로의 변환은 프론트에서 `src/constants/mood.ts`의 `moodFromValue()`가 전담한다 — 숫자→`Mood`의 **유일한 출처**다.
...
- `avatar_status` 조회 훅이 생기면 응답을 `moodFromValue()`에 통과시켜 store/컴포넌트에 `Mood`로 넘긴다. 소비처(`AvatarHero`/`Live2DCharacter`/마이페이지 뱃지 등)는 이미 `Mood` 문자열을 받으므로 **바뀌지 않는다**.
