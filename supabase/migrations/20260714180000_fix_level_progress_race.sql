-- user_level_progress 전진 로직이 select 후 별도 update(TOCTOU)라, 같은 요청이 동시에
-- 두 번 들어오면(네트워크 재시도 등) stage가 한 번에 2씩 올라갈 수 있었다.
-- user_progress 쪽처럼 UPDATE 문 안에서 조건(stage = p_stage)을 다시 검사하는
-- 원자적 패턴으로 통일한다.
