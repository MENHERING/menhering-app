-- #98: 커리큘럼을 끝까지 클리어해도 다음 난이도로 진행되지 않던 버그 수정.
-- user_level_progress.stage는 스테이지 클리어마다 계속 전진하지만, 잠금 판단 기준인
-- user_progress.level(실력 단계)은 레벨테스트로 배정된 뒤 자동으로 올라간 적이 없었다.
-- 배정된 레벨의 마지막 스테이지를 처음 클리어하면 user_progress.level을 다음 단계로 승급한다.
-- 순서는 src/constants/level.ts의 LEVELS를 미러링(DB엔 순서 테이블이 없어 하드코딩) —
-- 그쪽이 바뀌면 아래 CASE도 같이 바꿔야 한다.
...
where up.level in ('입문', '초급', '중급', '고급')
  and exists (
    ...
    where ulp.user_id = up.user_id
      and ulp.level = up.level
      and ulp.stage > (select max(q.stage) from questions q where q.level = up.level)
  );
