-- chk_review_status 제약조건이 팀 합의로 한글 기준('미복습'/'복습완료')으로 변경되었다
-- (이전엔 영문 'unreviewed'/'reviewed'만 허용 → 지금은 한글만 허용, 확인: pg_get_constraintdef).
-- 이 RPC는 여전히 영문을 넣고 있어 매 제출마다 제약조건 위반(23514)으로 실패하던 문제를 고친다.
