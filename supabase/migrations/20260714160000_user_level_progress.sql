-- 레벨별 독립 진행도. user_progress는 유저당 한 줄(현재 레벨+스테이지 포인터 하나)만 가져서
-- "내 레벨보다 낮은 레벨도 처음부터 따로 풀어나갈 수 있어야 한다"는 기획을 지원할 수 없었다.
-- 이 테이블이 레벨별 stage 포인터를 독립적으로 갖는다.
create table if not exists user_level_progress (
  user_id uuid not null references auth.users(id),
  level varchar not null,
  -- 그 레벨에서 현재 몇 번 스테이지에 있는지(1부터 시작). clearedCount = stage - 1.
...
create policy "본인 레벨 진행도 조회" on user_level_progress
  for select to authenticated using (user_id = (select auth.uid()));

-- submit_quiz_result를 레벨별 독립 진행도 기준으로 다시 작성.
-- p_level의 stage 포인터만 전진시키고, 다른 레벨의 진행도는 건드리지 않는다.
-- user_progress(level/stage)는 "레벨테스트로 배정된 레벨" 표시·잠금 기준으로만 남기고,
-- 배정 레벨을 플레이할 때는 계속 같이 갱신해 하위 호환을 유지한다.
...
begin
  ...
  -- 4) 레벨별 독립 진행도 갱신. 행이 없으면 1번 스테이지로 새로 만들고(첫 시도), 있으면 그대로 둔다.
  insert into user_level_progress (user_id, level, stage) values (v_user_id, p_level, 1)
  on conflict (user_id, level) do nothing;

  select stage into v_level_stage from user_level_progress
  where user_id = v_user_id and level = p_level;

  -- 지금 이 stage가 "그 레벨에서 다음으로 풀어야 할 스테이지"이고 성공했을 때만 전진시킨다.
  -- 이미 지나온 스테이지를 복습으로 다시 풀었을 땐(v_level_stage > p_stage) 건드리지 않는다.
  if v_is_success and v_level_stage = p_stage then
    update user_level_progress
      set stage = stage + 1, updated_at = now()
      where user_id = v_user_id and level = p_level;
  end if;

  -- 5) XP·스트릭은 레벨 무관 전역 누적치라 user_progress에 그대로 쌓는다.
  ...
  update user_progress
    set xp = xp + (v_xp_per_correct * v_correct_count),
  ...
  -- 6) 행복도 갱신 (avatars를 거쳐 avatar_status로 — user_id 직결 아님)
