-- #102: 결과 화면 행복도 게이지 상승 애니메이션이 +5%p라 체감이 잘 안 된다는 피드백으로
-- 스테이지 첫 클리어당 행복도 증가폭을 5 → 10으로 올린다.
--
-- 주의: 이 파일은 fix/#98/curriculum-level-up 브랜치의 20260721100000_advance_level_on_curriculum_clear.sql
-- (아직 develop 미병합, 레벨 승급 로직 포함)을 그대로 베이스로 삼아 v_mood_gain_per_stage 한 줄만
-- 바꿨다. #98와 #102 중 나중에 머지되는 쪽이 먼저 머지된 쪽의 submit_quiz_result 정의를
-- create or replace로 덮어쓰므로, 두 마이그레이션이 서로의 변경분(레벨 승급 / 행복도 증가폭)을
-- 잃지 않는지 두 PR을 머지하는 시점에 반드시 다시 확인해야 한다.
...
declare
  ...
  v_mood_gain_per_stage constant integer := 10;
...
begin
  ...
  -- 5-1) 배정된 레벨의 마지막 스테이지를 처음 깼을 때만 승급(복습 클리어는 조건에서 제외).
  if v_is_first_clear and v_is_final_stage and v_assigned_level = p_level then
    v_next_level := case v_assigned_level
      when '입문' then '초급'
      when '초급' then '중급'
      when '중급' then '고급'
      when '고급' then '전문가'
      else null -- 이미 최고 단계면 더 올릴 데 없음
    end;
  end if;
  ...
  update user_progress
    set xp = xp + v_xp_reward,
        ...
        level = coalesce(v_next_level, level)
    where user_id = v_user_id;

  -- 5-2) 코인은 첫 클리어일 때만 지급.
  ...
  -- 일부만 반영되거나(예: 95→100은 +10이 아니라 +5), 아바타/상태 행 자체가 없어 아예 반영이
